...
namespace Styx.Bot.CustomBots
{
    public class CombatBot : BotBase
    {
        ...
        public override Composite Root => _root ?? (_root = new PrioritySelector(
            CreateCombatBehavior()
            // Uncomment if follow behavior is needed
            // , CreateFollowBehavior()
        ));

        public override void Initialize() => BotEvents.Player.OnMapChanged += Player_OnMapChanged;

        private void Player_OnMapChanged(BotEvents.Player.MapChangedEventArgs args) => _root = null;
        ...
        private static void IncludeTargetsFilter(List<WoWObject> incomingUnits, HashSet<WoWObject> outgoingUnits)
        {
            ...
            if (me.GotTarget && target?.Attackable == true)
            {
            ...
            get
            {
                ...
                if (_followMe == null || !_followMe.IsValid)
                {
                    if (StyxWoW.Me.IsInInstance)
                    {
                        for (var i = 1; i < 5; i++)
                        {
                            var role = Lua.GetReturnVal<string>($"return UnitGroupRolesAssigned('party{i}')", 0);
                            if (role == "TANK")
                            {
        ...
        private static void _followMe_OnInvalidate() => _followMe = null;
